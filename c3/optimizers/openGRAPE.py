...
class openGRAPE(Optimizer):
    """
    open GRAPE method to optimize pulses for open quantum system using ODE solver.
    ...
    def goal_run_with_grad(self, current_params):
        goal, grad = self.goal_function(current_params)
        return goal, grad
    ...
    def optimize_controls(self, setup_log: bool = True) -> None:
        ...
        try:
            self.algorithm(
                x_init,
                fun=None,
                fun_grad=self.fct_to_min_with_grad,
    ...
    def goal_run(self, current_params: tf.Tensor) -> tf.float64:
        raise NotImplementedError(
            "Right now only implemented for ODE solver. Please specify an ODE solver."
        )
    ...
    def goal_run_ode(self, current_params: tf.Tensor) -> tf.float64:
        ...
        grads_dict = result["grads"]
        chans = [i[0].split("-")[1] for i in self.pmap.get_opt_map()]
        grads = [grads_dict[chan] for chan in chans]
